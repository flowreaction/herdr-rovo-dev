...
1. `bin/check-deps` - a preflight that verifies `bash`, `jq`, `grep`, `sed`,
   and `python3` are on `PATH`. Herdr ships no bundled runtime and executes
   plugin commands directly on the host, so a missing tool fails the
   **install** with a clear message and install hint instead of failing
   mysteriously on the first scan.
...
3. `bin/install-rovo-hooks --auto` - best-effort automatic hook install, so
   ...
   `yq` is an optional dependency (unlike the required tools above), and the
   Rovo config may not exist yet if Rovo hasn't been run once on this
...
- `bash`, `jq`, `grep`, `sed`, and `python3` on `PATH`
